...
from ..serializers import DiscoverParamsSerializer

class DiscoverView(views.APIView):
    """
    API view for content discovery with filtering
    """

    def get(self, request):
        # Validate and parse parameters
        serializer = DiscoverParamsSerializer(data=request.query_params)
        ...
        print(f"Raw query params: {request.query_params}")
        print(f"Validated params: {params}")

        # Make request to TMDB API
        tmdb_service = TMDBService()
        response = tmdb_service.discover(media_type, params)
